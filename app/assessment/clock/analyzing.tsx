...
import { analyzeClockDrawing } from '../../../lib/openai';
import { uploadImageToOSS } from '../../../lib/oss';
import { addAssessment, useAppState, getPatientContext } from '../../../lib/profileStore';
import { useTranslation } from '../../../lib/i18n';

const STEP_KEYS = ['clockStepCapturing', 'clockStepUploading', 'clockStepAnalyzing'] as const;
...
export default function ClockAnalyzing() {
  ...
  useEffect(() => {
    ...
    async function run() {
      try {
        const { clockImageUri } = getState();
        if (!clockImageUri) throw new Error('No drawing found');

        // Step 1: capturing (already captured, just animate)
        setStepIdx(0);
        await new Promise((r) => setTimeout(r, 500));

        // Step 2: uploading
        setStepIdx(1);
        const imageUrl = await uploadImageToOSS(clockImageUri);
        if (!mounted) return;
        setClockImageUrl(imageUrl);

        // Step 3: analyzing
        setStepIdx(2);
        const patient = getPatientContext(profile);
        const report = await analyzeClockDrawing(imageUrl, locale, patient);
        if (!mounted) return;
        setReport(report);
        // Use the AI analysis paragraph as the "transcript" so the existing
        // report screen and history list render the clinical narrative.
        setTranscript(report.analysis);

        await addAssessment({
          kind: 'clock',
          report,
          transcript: report.analysis,
          imageUrl,
        });
      ...
      >
        {errMsg ? (
          <>
            ...
            <Text
              style={{ ...type.bodyMd, color: colors.onSurfaceVariant, textAlign: 'center' }}
            >
              {errMsg}
            </Text>
            <Pressable
              onPress={() => router.replace('/assessment/clock/draw')}
              style={{
                backgroundColor: colors.primary,
